...
function addDay(profile, index, left) {
    ...
    if (artikelName) {
        ...
        if (artikelBodyTexts) {
            ...
            var lowestParagraphPoint = getElement(tb_text_artikelLead).y2;

            for (var i = 0; i < artikelBodyTexts.length; i++) {
...
function getMultipleTexts(fullText, type) {
    ...
    // var partials = fullText.split('*' + type + '*')[2] || 'unknown';
    var partials = fullText.split('*' + type + '*').pop().split('*/' + type + '*')[0];

    var partial = fullText.split('*' + type + '*');
    var result = [];
    for (var i = 0; i < partial.length; i++) {

        var split = partial[i].split('*/' + type + '*');
        if (split.length > 1) {
            result.push(split[0]);
        }
    }
